    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl border border-white/10 bg-[#1C1E24] p-6">
        <div className="flex items-center justify-between">
          ...
          <button
            onClick={onClose}
            className="text-[#9A9CA6] hover:text-[#F5F3EF]"
          >
            ✕
          </button>
        </div>
        ...
        <form onSubmit={handleSave} className="mt-5 flex flex-col gap-4">
          <div className="flex flex-col gap-1.5">
            <label className="text-xs text-[#9A9CA6]">Class Name</label>
            <input
              name="name"
              value={form.name || ""}
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-xs text-[#9A9CA6]">Image URL</label>
            <input
              name="image"
              value={form.image || ""}
              onChange={handleChange}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="flex flex-col gap-1.5">
              <label className="text-xs text-[#9A9CA6]">Category</label>
              <select
                name="category"
                value={form.category || ""}
                onChange={handleChange}
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-xs text-[#9A9CA6]">Difficulty</label>
              <select
                name="difficulty"
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="flex flex-col gap-1.5">
              <label className="text-xs text-[#9A9CA6]">Duration</label>
              <input
                name="duration"
                value={form.duration || ""}
                onChange={handleChange}
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-xs text-[#9A9CA6]">Price ($)</label>
              <input
                name="price"
                type="number"
                value={form.price || ""}
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-xs text-[#9A9CA6]">Schedule</label>
            <input
              name="schedule"
              value={form.schedule || ""}
              onChange={handleChange}
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-xs text-[#9A9CA6]">Description</label>
            <textarea
              name="description"
              rows={3}
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-xs text-[#6B6D78]">
                    Booked: {new Date(s.bookedAt).toLocaleDateString()}
                  </p>
                </div>
              </div>
            ))
      ...
      <div className="mt-6 overflow-hidden rounded-2xl border border-white/10">
        <table className="w-full text-left text-sm">
          ...
          <tbody>
            {loading ? (
              <tr>
                <td colSpan={5} className="px-5 py-8 text-center text-[#6B6D78]">
                  Loading...
                </td>
              </tr>
            ) : classes.length === 0 ? (
              <tr>
                <td colSpan={5} className="px-5 py-8 text-center text-[#6B6D78]">
                  No classes yet. Add one from "Add Class"!
                </td>
              </tr>
            ) : (
              classes.map((c) => (
                <tr key={c._id} className="border-t border-white/10 bg-[#14151A]">
                  ...
                  <td className="px-5 py-4 font-semibold text-[#FF5B3C]">${c.price}</td>
                  <td className="px-5 py-4">
                    <span
                      className={`rounded-full px-3 py-1 text-xs font-medium ${
                        STATUS_STYLES[c.status] || "bg-white/10 text-[#9A9CA6]"
                      }`}
                    >
                      {c.status}
                    </span>
